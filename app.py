...
df_roll=pd.DataFrame(y_test.reshape(-1,1)[-73:],columns=['real_roll'])#,pred.reshape(-1,1)[-10:-1]
df_roll['pred_roll']=pd.DataFrame(pred.reshape(-1,1)[-73:],columns=['pred_roll'])
...
df_roll_filtered = df_roll[abs(df_roll['pred_roll_%change']) > 0.022 ]# portafoglio filtrato per predict > di tot per cento
