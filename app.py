...
df_roll=pd.DataFrame(y_test.reshape(-1,1)[-88:],columns=['real_roll'])#,pred.reshape(-1,1)[-10:-1]
df_roll['pred_roll']=pd.DataFrame(pred.reshape(-1,1)[-88:],columns=['pred_roll'])
...
df_roll_filtered = df_roll[abs(df_roll['pred_roll_%change']) > 0.002 ]# portafoglio filtrato per predict > di tot per cento
...
df_roll_filtered['real']=((df_roll['real_roll_%change']*10000).cumsum()+10000+650)-10000#,color='black',label='REALE')
