...
st.subheader(' Artificial IntelligenceTrading Signal Generator on WTI')
import numpy as np
import pandas as pd
import pickle
import json
...
df_roll=pd.DataFrame(y_test.reshape(-1,1)[-133:],columns=['real_roll'])#,pred.reshape(-1,1)[-10:-1]
df_roll['pred_roll']=pd.DataFrame(pred.reshape(-1,1)[-133:],columns=['pred_roll'])
