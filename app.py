...
yhat2=model.predict(((x_new_data3.flatten().reshape(-1,7))[-9:]))##
...
df_roll=pd.DataFrame(y_test.reshape(-1,1)[-28:],columns=['real_roll'])#,pred.reshape(-1,1)[-10:-1]
df_roll['pred_roll']=pd.DataFrame(pred.reshape(-1,1)[-28:],columns=['pred_roll'])
...
data=[[df_roll_filtered['system'].values.reshape(-1,1)],[df_roll_filtered['real'].values.reshape(-1,1)]]
#chart_data = pd.DataFrame(
    #[df_roll_filtered['system'].values],[df_roll_filtered['real'].values],
    #columns=['merlin_system','real'])
chart_data = pd.DataFrame(data,columns=['merlin_system','real'])    
