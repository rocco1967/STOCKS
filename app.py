...
st.subheader(f' TODAY ML FILTERED POSITION {yhat2[-1:]}')
...
df_roll=pd.DataFrame(y_test.reshape(-1,1)[-83:],columns=['real_roll'])#,pred.reshape(-1,1)[-10:-1]
df_roll['pred_roll']=pd.DataFrame(pred.reshape(-1,1)[-83:],columns=['pred_roll'])
