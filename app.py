...
st.write(((x_new_data3.flatten().reshape(-1,7)))[-10:])
...
df_roll=pd.DataFrame(y_test.reshape(-1,1)[-160:],columns=['real_roll'])#,pred.reshape(-1,1)[-10:-1]
df_roll['pred_roll']=pd.DataFrame(pred.reshape(-1,1)[-160:],columns=['pred_roll'])
